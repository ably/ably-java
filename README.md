...
* Create a branch for the release, named like `release-1.0.14`
...
* Run [`github_changelog_generator`](https://github.com/skywinder/Github-Changelog-Generator) to update the [CHANGELOG](./CHANGELOG.md): `github_changelog_generator -u ably -p ably-java --header-label="# Changelog" --release-branch=release-1.0.14 --future-release=v1.0.8` 
...
* Go to the home page for the package; eg https://bintray.com/ably-io/ably/ably-java. Select [New version](https://bintray.com/ably-io/ably/ably-java/new/version), enter the new version such as "1.0.14" in name and save
* Run `./gradlew java:assembleRelease` locally to generate the files
* Open local relative folder such as `./java/build/release/1.0.14/io/ably/ably-java/1.0.14`
* Then go to the new version in JFrog Bintray; eg https://bintray.com/ably-io/ably/ably-java/1.0.14, then click on the link to upload via the UI in the "Upload files" section
* Type in `io/ably/ably-java/1.0.14` into "Target Repository Path" ensuring the correct version is included. The drag in the files in `java/build/release/1.0.14/`. Upload all the `.jar` files and the `.pom` file.
* You will see a notice "You have 4 unpublished item(s) for this version", make sure you click "Publish". Wait a few minutes and check that your version has all the necessary files at https://bintray.com/ably-io/ably/ably-java/1.0.14?sort=&order=#files/io/ably/ably-java/1.0.14 for example.
...
`./android/build/release/1.0.14/io/ably/ably-android/1.0.14`. In this case upload the `.jar` files, the `.pom` file and the `.aar` file.

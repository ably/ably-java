...
public class MessageAnnotations {
    ...
    public MessageAnnotations() {
        // Protocol v4: Summary is non-nullable, initialize with empty summary
        this.summary = new Summary(new HashMap<>());
    }

    public MessageAnnotations(Summary summary) {
        // Protocol v4: Ensure summary is never null
    ...
    /**
     * Protocol v4 enhancement: ensure that summary is properly initialized.
     * Based on the JavaScript implementation, annotations should always have a summary
     * object when one doesn't exist, to maintain consistency with annotation handling.
     */
    private void ensureSummaryExists() {
        // If summary is null, create an empty one for Protocol v4 consistency
        // This aligns with JavaScript implementation behavior (TM2u and TM8a specs)
        if (summary == null) {
            summary = new Summary(new HashMap<>());
        }
    }
